# -*- coding: utf-8 -*-
"""
Created on Sat Jun 29 22:35:34 2019
...
def gridRF(X_train,Y_train,X_val,Y_val):
    ...
    min_samples_leaf = np.linspace(0.00001, 0.5, 2)
    min_samples_split = np.linspace(0.00001, 1.0, 2)
    bootstrap = [True,False]
    
    best_erro = 9999999999999999999999999999999999999999**999
...
pred_hmedian = np.median((mlp.pred_mlp, ar.pred_AR), axis = 0)
erro_hmedian = metrics.mean_squared_error(ar.test_target, pred_hmedian)

dimension = 1
datafile = pred_hmedian
...
plt.plot(x, pred_target, 'r--', label = 'Real Hibrido')
plt.plot(x, pred_rf, label = 'RF-hibrido predicted')
plt.legend()
plt.figure